...
class PlacementForm(forms.Form):
    batch = forms.ModelChoiceField(
        queryset=Batch.objects.none(), label='Партия')
    rack = forms.ModelChoiceField(
        queryset=Rack.objects.filter(is_active=True), label='Стеллаж')
    quantity = forms.IntegerField(
        min_value=1, label='Количество для размещения')
        ...
        if batch_id:
            ...
            try:
                ...
                self.fields['quantity'].widget.attrs[
                    'placeholder'] = f'Максимум: {remaining_quantity}'
    ...
    def clean(self):
        ...
        if batch and quantity:
            ...
            if quantity > remaining:
                raise ValidationError(
                    f'Нельзя разместить больше товара, чем осталось в партии. Доступно: {remaining}')
        ...
        if batch and rack and quantity:
            ...
            if not rack.can_fit_product(product):
                raise ValidationError(
                    f'Товар не помещается на выбранный стеллаж по габаритам')

            # Проверяем по весу
            if product.weight * quantity > rack.available_weight():
                raise ValidationError(
                    f'Превышена допустимая нагрузка на стеллаж. Доступно: {rack.available_weight()} кг')

            # Проверяем по объему
            product_volume = product.get_volume() * quantity
            if product_volume > rack.available_volume():
                raise ValidationError(
                    f'Недостаточно места на стеллаже. Доступно: {rack.available_volume()/1000:.2f} л')
...
class IssueForm(forms.Form):
    product = forms.ModelChoiceField(
        queryset=Product.objects.all(), label='Товар')
