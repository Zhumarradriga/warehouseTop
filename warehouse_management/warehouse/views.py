...
class DashboardView(View):
    def get(self, request):
        ...
        total_quantity = Placement.objects.filter(is_active=True).aggregate(total=Sum('quantity'))['total'] or 0
        
        # Товары с низким остатком
        low_stock_products = []
        for product in Product.objects.all():
            quantity = Placement.objects.filter(product=product, is_active=True).aggregate(total=Sum('quantity'))['total'] or 0
...
class BatchCreateView(View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            batch = form.save()
            messages.success(request, f'Партия товара "{batch.product.name}" успешно создана')
...
class BatchListView(ListView):
    ...
    def get_queryset(self):
        ...
        queryset = queryset.annotate(
            placed_quantity=Sum('placement__quantity', filter=Q(placement__is_active=True))
        )
        return queryset

class SuggestRacksView(View):
...
class IssueProductView(View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            if remaining_quantity > 0:
                messages.warning(request, f'Не удалось выдать весь запрошенный объем. Выдано: {quantity - remaining_quantity} из {quantity}')
            else:
                messages.success(request, f'Успешно выдано {quantity} ед. товара {product.name}')
            
            return redirect('warehouse:issue_product')
        
        return render(request, 'warehouse/issue_form.html', {'form': form})

class CheckCapacityView(View):
    def get(self, request):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            racks = sorted(available_racks, key=lambda x: x.available_volume(), reverse=True)
            remaining_quantity = quantity
            suggested_racks = []
            for rack in racks:
                if remaining_quantity <= 0:
            ...
            placed_quantity = quantity - remaining_quantity  # Вычисляем размещенное количество
...
class WarehouseJournalView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['is_in_selected'] = self.request.GET.get('operation_type') == 'IN'
        context['is_out_selected'] = self.request.GET.get('operation_type') == 'OUT'
        return context